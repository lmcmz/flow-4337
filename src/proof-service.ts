...
import { flowAccountProofService, FlowAccountProofData } from './flow-account-proof';
import { offChainZKPGenerator, OffChainZKProof, ProofGenerationRequest } from './off-chain-zkp';
...
export class OffChainProofService {
  ...
  private setupRoutes(): void {
    ...
    // Register Flow account commitment
    ...
    this.app.get('/api/docs', (req, res) => {
      res.json({
        ...
        endpoints: {
          ...
          'POST /api/proof/generate': 'Generate off-chain ZK proof',
          'POST /api/proof/verify': 'Verify ZK proof',
          'POST /api/account/register': 'Register Flow account commitment',
  ...
  private async handleGenerateProof(req: express.Request, res: express.Response): Promise<void> {
    try {
      ...
      // Validate input
      if (!flowAccountProof || !challenge || !messageHash) {
        res.status(400).json({
          success: false,
          error: 'Missing required fields: flowAccountProof, challenge, messageHash'
        });
        return;
      }
  ...
  private async handleAccountRegistration(req: express.Request, res: express.Response): Promise<void> {
    try {
      ...
      // Validate input
      if (!flowAccountProof || !challenge || !erc4337AccountAddress) {
        res.status(400).json({
          success: false,
