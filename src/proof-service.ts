...
export class OffChainProofService {
  ...
  private setupRoutes(): void {
    ...
    this.app.get('/api/docs', (req, res) => {
      res.json({
        ...
        version: '1.0.0',
        description: 'Privacy-preserving Flow account ownership proof generation',
        endpoints: {
          'POST /api/challenge': 'Generate challenge for Flow account proof',
          'POST /api/proof/generate': 'Generate off-chain ZK proof (auto-detects legacy/COA)',
          'POST /api/proof/generate-coa': 'Generate ZK proof using official Flow EVM COA structure',
          'POST /api/proof/verify': 'Verify ZK proof',
          'POST /api/account/register': 'Register account commitment (auto-detects legacy/COA)',
          'POST /api/account/register-coa': 'Register COA ownership proof commitment',
          'POST /api/proof/batch': 'Batch generate proofs',
          'GET /api/stats': 'Get service statistics',
          'GET /health': 'Health check'
        },
        supportedProofTypes: {
          'Legacy Flow Account Proof': 'flowAccountProof field (backward compatibility)',
          'Official Flow EVM COA Proof': 'coaOwnershipProof field (recommended)'
        }
      });
    });
  }
   ...
   */
  private async handleGenerateChallenge(req: express.Request, res: express.Response): Promise<void> {
    try {
   ...
   */
  private async handleGenerateCOAProof(req: express.Request, res: express.Response): Promise<void> {
    try {
      const {
        coaOwnershipProof,
        challenge,
      ...
      if (!coaOwnershipProof || !challenge || !messageHash) {
        res.status(400).json({
          success: false,
          error: 'Missing required fields: coaOwnershipProof, challenge, messageHash'
        });
        return;
      }
      ...
      const userOpSignature = offChainZKPGenerator.formatForUserOperation(proof);
      ...
      res.status(500).json({
        success: false,
        error: `COA proof generation failed: ${error.message}`
      });
    }
  }
   ...
   */
  private async handleGenerateProof(req: express.Request, res: express.Response): Promise<void> {
    try {
      const {
        flowAccountProof,
        coaOwnershipProof,
   ...
   */
  private async handleVerifyProof(req: express.Request, res: express.Response): Promise<void> {
    try {
   ...
   */
  private async handleCOARegistration(req: express.Request, res: express.Response): Promise<void> {
    try {
      const {
        coaOwnershipProof,
        challenge,
      ...
      if (!coaOwnershipProof || !challenge || !erc4337AccountAddress) {
        res.status(400).json({
          success: false,
          error: 'Missing required fields: coaOwnershipProof, challenge, erc4337AccountAddress'
        });
        return;
      }
   ...
   */
  private async handleAccountRegistration(req: express.Request, res: express.Response): Promise<void> {
    try {
      const {
        flowAccountProof,
        coaOwnershipProof,
   ...
   */
  private async handleBatchGenerateProofs(req: express.Request, res: express.Response): Promise<void> {
    try {
  ...
  public async start(): Promise<void> {
    ...
    return new Promise((resolve) => {
      const server = this.app.listen(this.config.port, () => {
        console.log(`🚀 Off-Chain Proof Service running on port ${this.config.port}`);
...
export class OffChainProofServiceClient {
  ...
  async verifyProof(proof: OffChainZKProof): Promise<{ success: boolean; valid: boolean }> {
  ...
  async registerAccount(request: AccountRegistrationRequest): Promise<AccountRegistrationResponse> {
