...
export class FlowIntegration {
   ...
   */
  async signMessage(message: string, account: FlowAccount): Promise<FlowSignature> {
  ...
  async getCurrentUser(): Promise<FlowAccount | null> {
    try {
      ...
      return {
        ...
        keyIndex: account.keys[0].keyIndex,
  ...
  getAccountDetailsForZKP(account: FlowAccount) {
    ...
    return {
      ...
      publicKeyX: '0x' + publicKeyPoint.getX().toString('hex').padStart(64, '0'),
      publicKeyY: '0x' + publicKeyPoint.getY().toString('hex').padStart(64, '0'),
      privateKey: '0x' + account.privateKey
    };
  }
}
